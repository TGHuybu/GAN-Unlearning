...
attribute_label_df = pd.read_csv("./CelebA/Anno/list_attr_celeba.txt", delim_whitespace=True,dtype='object',header=None)
...
path_toLoad = "./models/model_7_epoch.pt"
...
for minibatch in range(0,total_Batches):
    ...
    batch_imageTensor = torch.cuda.FloatTensor(bs,3,256,256)
    ...
    for imageNumber in range((minibatch*default_bs)+startTestIndex,(minibatch*default_bs)+bs+startTestIndex):
        ...
        pil_img = Image.open("./CelebA/Img/img_align_celeba/"+imageName).convert('RGB')
