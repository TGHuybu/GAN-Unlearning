...
class EWC(nn.Module):
    ...
    def update_fisher_optpar(self, model, model1, current_itr, dataset, sample_size, batch_size=32, consolidate=True):
        if consolidate:
            if self.args.online:
                current_itr = 1
        ...
        for i in range(sample_size // batch_size):
            ...
            label = torch.full((batch_size, ), 0.99, dtype=torch.float).to(device)
            fake_data = model(noise)
            output = model1(fake_data).view(-1)
            losses.append(torch.log(output))
            print(len(losses))
